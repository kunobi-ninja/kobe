...
const expectedAuthorityExpressions = [
	...
	`request.resource.resource != 'clusterleases' || request.operation != 'UPDATE' || !has(oldObject.metadata.finalizers) || !oldObject.metadata.finalizers.exists(f, f == 'kobe.kunobi.ninja/teardown-receipt-retention') || (has(object.metadata.finalizers) && object.metadata.finalizers.exists(f, f == 'kobe.kunobi.ninja/teardown-receipt-retention')) || (has(oldObject.status) && has(oldObject.status.teardownAcknowledgement) && has(object.status) && has(object.status.teardownAcknowledgement) && object.status.teardownAcknowledgement == oldObject.status.teardownAcknowledgement) || (request.userInfo.username == ${JSON.stringify(controlPlaneUsername)} && has(oldObject.metadata.annotations) && 'kobe.kunobi.ninja/release-quarantine' in oldObject.metadata.annotations && oldObject.metadata.annotations['kobe.kunobi.ninja/release-quarantine'] == oldObject.metadata.uid && has(oldObject.status) && has(oldObject.status.phase) && oldObject.status.phase == 'Quarantined')`,
	...["clusterleases", "clusterinstances"].map(
		(resource) =>
			`request.resource.resource != '${resource}' || !(request.userInfo.username in [${JSON.stringify(controlPlaneUsername)}, ${quotedAuthority}]) || request.operation == 'DELETE' || !has(object.metadata.annotations) || !('kobe.kunobi.ninja/release-quarantine' in object.metadata.annotations) || (oldObject != null && has(oldObject.metadata.annotations) && 'kobe.kunobi.ninja/release-quarantine' in oldObject.metadata.annotations && oldObject.metadata.annotations['kobe.kunobi.ninja/release-quarantine'] == object.metadata.annotations['kobe.kunobi.ninja/release-quarantine'])`,
	),
