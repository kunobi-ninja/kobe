...
- `docs/kobe-docs/` — the site source (MDX + `meta.json`). Operator runbooks live under `operate/`.
- `docs/guides/` — short stubs that point at the site pages (plus unpublished drafts). Do not add new operator guides here; they will not appear on kunobi.com.
...
| User-facing config / runbook / how-to | `docs/kobe-docs/` (site), including `operate/` |
...
`docs/` is for content an external kobe adopter needs to read. The site (`docs/kobe-docs/`) qualifies, including operator runbooks under `operate/`. Algorithm internals, code-level decision rationale, and one-off design notes do not — they belong inline with the code or in the Obsidian vault.
