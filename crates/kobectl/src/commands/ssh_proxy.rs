...
pub struct HostSpec {
    ...
    /// The lease alias: the full host name, lowercased.
    pub alias: String,
}
                ...
                None => config.default_pool.clone().ok_or_else(|| {
                    anyhow::anyhow!(
                        "{} names no pool and the target has no default pool; use kobe-<pool>-<name> \
                         (pools: {}) or set one with `kobe config set <target> --default-pool <pool>`",
                        spec.alias,
...
mod tests {
    ...
    fn invalid_aliases_are_refused() {
        ...
        assert!(parse_host("kobe-a.b", &[]).is_err());
