...
visible terminal. Callers: [Desktop](/docs/getting-started/desktop). Firefox is registered for HTTP and HTTPS links, so OAuth
...
`mise.toml`. Both images use the same release tags and runner. Pin a release
...
template:
  ...
  containers:
    - name: workspace
      image: zondax/kobe-agent-workspace-gui:v0.47.0
  exposedPorts:
    - name: desktop
      container: workspace
      port: 6080
