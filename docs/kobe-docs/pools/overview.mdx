...
spec:
  ...
  # Velero golden image snapshot. See /docs/operate/golden-images.
  snapshot:
    enabled: true
...
Rolling recycle during operator or spec upgrades is `spec.upgradePolicy`. See [Upgrades](/docs/operate/upgrades). Golden-image restores are [Velero snapshots](/docs/operate/golden-images).
