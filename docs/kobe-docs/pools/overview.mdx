...
spec:
  ...
  backend:
    type: k3s                 # k3s | k0s | vcluster | capi | vkobe
  ...
  # Velero golden image snapshot. Not validated; see /docs/operate/golden-images.
  snapshot:
    enabled: true
...
Rolling recycle during operator or spec upgrades is `spec.upgradePolicy`. See [Upgrades](/docs/operate/upgrades). Golden-image restores are [Velero snapshots](/docs/operate/golden-images), which are not validated yet.
