...
For golden image workflows, k3s can use a shared PostgreSQL datastore instead of embedded SQLite. The datastore itself works; the [golden image path](/docs/operate/golden-images) that motivated it is not validated. This enables `CREATE DATABASE ... TEMPLATE` to clone a pre-seeded database into new cluster pods, reducing addon setup time.
