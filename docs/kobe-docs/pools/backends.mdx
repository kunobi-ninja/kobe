...
|---|---|---|---|---|---|
| `spec.cluster` fields honored | All | All except `kubeletSharedMount` and `registryMirrors` | `version` only | None (uses `spec.backend.capi`) | None (uses `spec.backend.vkobe`) |
...
- **`spec.cluster` fields honored.** Fields a backend ignores are accepted without an error. On vcluster, `servers`, `agents`, `persistence`, `expose`, `taints`, `placement`, and `resources` have no effect; set chart values through `spec.backend.vcluster.values` instead. CAPI takes its whole configuration from `spec.backend.capi`.
...
`goldenTemplates` is accepted but has no effect today: the operator does not clone clusters from template databases. The [golden image path](/docs/operate/golden-images) is not validated either.
...
CAPI ignores `spec.cluster`. Everything it creates comes from `spec.backend.capi`. There is no CAPI leg in kobe's CI, so test it against your provider before relying on it.
...
**Pool spec coverage.** vcluster reads only `cluster.version` from `spec.cluster`. Configure everything else through `values`.
