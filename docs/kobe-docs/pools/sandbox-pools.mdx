...
GitHub CLI (`gh`), OpenSSH, Node, Codex, Claude Code, JSON/search/network
diagnostics, OpenSSL headers, and a C toolchain. Codex and Claude Code are
version-pinned image tools with no credentials or tokens. Each user signs in
inside their own lease. The workload runs as `65532:65532`, with
...
desktop. Both images use the same release tags and runner. Pin a release tag or
digest in your pool.
