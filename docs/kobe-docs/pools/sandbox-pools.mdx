...
<Callout type="warn">
  ...
  `agentSandbox.mode=external` only after installing Agent Sandbox v0.5.6
...
- `external` installs, upgrades, and uninstalls nothing. The operator owns the
  Agent Sandbox v0.5.6 core, extensions, CRDs, controller, and webhook.

Kobe checks that all four consumed CRDs are established, serve the expected
`v1beta1` APIs, and expose the v0.5.6 WarmPool generation field. Validation is
GET-only: it does not inspect one particular Deployment topology and does not
create a temporary `SandboxClaim`. Consequently, API compatibility does not
prove that the external controller is healthy. The actual Claim for a requested
lease still fails closed during provisioning, and its administrator-declared
readiness command must pass before that lease becomes Ready.
...
<Callout type="info">
  If a cluster still runs v0.5.4 or another incompatible release, upgrade it to
  v0.5.6 before enabling `external`. Update operator-authored child bootstrap
  content at the same time and recycle or upgrade older children according to
  your pool policy. Kobe never performs this upgrade; it rejects a child whose
...
1 MiB of stdout and 1 MiB of stderr per execution, allows at most 8 executions
to cross `startedAt` over one Sandbox target's lifetime, and accepts at most 256
idempotency keys over one Sandbox lifetime. The stricter started-execution bound
is required because same-UID runner state cannot safely prove that capacity is
reusable. Requests beyond either execution limit return `429 Too Many Requests`.
