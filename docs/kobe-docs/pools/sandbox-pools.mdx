...
starting a process and writing that private execution spool.
...
process Kobe started. Lease release first closes new access, then asks every
recorded runner to cancel and prove its process group absent. If a lost
supervisor makes that proof impossible, Kobe retains the execution record and
capacity, destroys the exact Sandbox target, and retires them only after target
absence is proven. Execution records are deleted with that Sandbox rather than
kept as history after it ends.
