...
Sandbox quota slots and aliases are coordination `Lease` objects. When Sandbox
mode is enabled, the chart creates a dedicated namespace named from the
immutable Helm release identity (`<release>-sandbox-ledger-<hash>`) with:
...
before teardown is proved. Remove retained ledger resources only after every
Sandbox lease and reservation has been verified absent. The admission policy
