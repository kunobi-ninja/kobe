...
sandbox through [`kobe ssh-proxy`](/commands/ssh) without any port being
...
The same image also carries the [Tauri 2 Linux system prerequisites](https://v2.tauri.app/start/prerequisites/#linux),
software rendering, Xvfb, Openbox, noVNC, Firefox, and a visible terminal.
Firefox is registered for HTTP and HTTPS links, so OAuth flows opened by
desktop applications stay inside the noVNC desktop. There is no separate GUI
image: the desktop is installed but never started, so a headless lease pays
only the image size and nothing at runtime. Node, Rust, package managers, and
other project tools still come from the project's `mise.toml`. Pin a release
tag or digest in your pool.
...
template:
  ...
  containers:
    - name: workspace
      image: zondax/kobe-agent-workspace:v0.47.0
  exposedPorts:
    - name: desktop
      container: workspace
      port: 6080
