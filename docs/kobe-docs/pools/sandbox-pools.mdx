...
`zondax/kobe-agent-workspace` is the single development workspace image. It
includes `kobe-runner`, `mise`, `tmux`, Git, GitHub CLI (`gh`), OpenSSH, Node,
Codex, Claude Code, JSON/search/network diagnostics, OpenSSL headers, a C
toolchain, the [Tauri 2 Linux system prerequisites](https://v2.tauri.app/start/prerequisites/#linux),
software rendering, Xvfb, Openbox, noVNC, Firefox, broader Noto/Liberation/emoji
font coverage, and a visible terminal. Firefox is registered for HTTP and HTTPS
links, so OAuth flows opened by desktop applications stay inside the noVNC
desktop. Codex and Claude Code are version-pinned image tools with no credentials
or tokens. Each user signs in inside their own lease. The workload runs as
`65532:65532`, with
...
and puts project tool shims on `PATH` for direct `kobe exec` commands.
...
template:
  ...
  containers:
    - name: workspace
      image: zondax/kobe-agent-workspace:v0.44.0
  exposedPorts:
    - name: desktop
      container: workspace
      port: 6080
...
Open `http://127.0.0.1:6080/vnc.html?autoconnect=1&resize=remote&quality=9&compression=0&path=websockify`.
...
The noVNC control bar includes a Clipboard panel. Browser clipboard access
requires a user gesture, so use that panel if `Cmd/Ctrl+V` is intercepted by
the local browser. Within the desktop, the PRIMARY selection and CLIPBOARD are
synchronized, and XKB is enabled for non-US and modifier-key layouts.
...
### Experimental KasmVNC transport

The image also includes KasmVNC as an opt-in acceptance-test transport. It has
its own browser client, sharper text-oriented encoding controls, and a
lease-local HTTP Basic credential generated at startup. Do not start it beside
the noVNC desktop: it owns a separate X display.

```bash
kobe exec workspace -- kobe-kasmvnc start
kobe port-forward workspace 6901:6901
kobe exec workspace -- kobe-kasmvnc url
```

The printed URL includes the generated local credential; do not share it.
`kobe-kasmvnc stop` removes that credential and all KasmVNC state. Its browser
listener binds `127.0.0.1`; startup sets the public address to loopback,
disables STUN, and refuses to continue if KasmVNC creates a UDP listener. Use
it only in a trusted isolated pool while we compare clipboard, keyboard, OAuth,
latency, and text sharpness with noVNC. noVNC remains the supported default
transport.

### SSH and cmux

The workspace image can start a lease-local OpenSSH server for terminal-first
clients. It creates an ED25519 host key at runtime, listens only on loopback
port 2222, and accepts only the public key supplied to that lease. The key,
server configuration, and host identity are removed by `stop` and disappear
when the lease is released.

```bash
# Authorize the public keys currently loaded in the local SSH agent.
ssh-add -L | kobe exec workspace --stdin -- kobe-workspace-ssh authorize
kobe exec workspace -- kobe-workspace-ssh start
kobe port-forward workspace 2222:2222

# The container account is named nonroot. Accept the per-lease host key once.
cmux ssh nonroot@127.0.0.1 -p 2222 -o StrictHostKeyChecking=accept-new
```

The pool must explicitly declare port 2222 as shown above; without that
declaration Kobe refuses the forward. The SSH server does not receive the
client's SSH agent, password authentication is disabled, and it cannot bind
outside the Sandbox loopback interface. Run `kobe-workspace-ssh stop` to remove
its credentials before release when an earlier teardown is useful.

Build locally with `docker buildx bake agent-workspace --load` (set
`PLATFORM=linux/arm64` on an ARM host). The `default` and `push` Bake groups
include this workspace image.
