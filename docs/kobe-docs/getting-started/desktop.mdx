...
Any sandbox whose pool image is `zondax/kobe-agent-workspace` can run a Linux desktop. The image includes Xvfb, Openbox, noVNC, Firefox, and the [Tauri 2 Linux prerequisites](https://v2.tauri.app/start/prerequisites/#linux). Firefox is registered for HTTP and HTTPS, so OAuth opened by a desktop app stays inside that browser.
