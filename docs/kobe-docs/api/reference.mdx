...
| `409` | The name exists as both a Cluster pool and a Sandbox pool (`reason: wrong_resource_kind`), the alias is held by another active lease, or the `idempotencyKey` was used for a different request |
| `429` | Concurrent lease limit reached (no `Retry-After`), or server overloaded (`Retry-After: 1`; retry) |
