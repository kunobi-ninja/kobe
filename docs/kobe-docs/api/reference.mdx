...
| Extend: `PATCH /v1/leases/:id` | Both `expires_at` and `expiresAt` for both kinds. Sandbox also returns `extensionsCount`, `maxExtensions` and `runningExecutions`. |
...
`expiresAt` carries the same value as `expires_at`, so one client parses the
Cluster and Sandbox extend answers the same way. A Sandbox id or alias is
