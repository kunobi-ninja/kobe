...
<Callout type="warn">
  ...
  mounted only when `agentSandbox.mode=managed|external` and Kobe certifies the
  pinned Agent Sandbox runtime with a real create/delete canary at startup.
  Disabled mode serves no Sandbox API, so it cannot admit a lease that no
  controller will reconcile.
</Callout>
...
```json
{
  ...
  "retry": false,
  "statusUrl": "/v1/sandbox-leases/sandbox-a1b2c3d4e5f6"
}
```
...
This server-side handoff does not make the create idempotent if the client loses
the final HTTP response itself. Until the create API accepts a caller-owned
idempotency key, do not blindly retry an ambiguous transport failure; reconcile
the caller's lease list first.
