...
```bash
kobe exec dev --detach --timeout 1h -- make release
```
...
the envelope carries the execution id and its current `state`, with no
`exitCode` yet.
...
with. For work that should follow the lease across extensions, run it in a
