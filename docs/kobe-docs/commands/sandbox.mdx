...
A detached command is still bound by `--timeout`, and no execution runs longer
than one hour. For longer work, run it in a
[persistent session](#sessions-that-survive-a-disconnect)
(`kobe attach dev --session build`), which the execution limit does not apply
to. `kobe run` never detaches, because it releases the sandbox when the command
returns.
