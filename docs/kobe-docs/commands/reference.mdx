...
| `kobe init` | Set this machine up for `ssh kobe-<pool>-<name>`: target, login, default pool, public key, `ssh_config` ([details](/commands/ssh)) |
| `kobe doctor` | Check everything `ssh kobe-…` depends on, without changing anything |
| `kobe ssh-proxy` | Serve an SSH connection to a sandbox on stdin/stdout, as an ssh `ProxyCommand` ([details](/commands/ssh)) |
| `kobe ssh-config` | Print the `ssh_config` block that routes `kobe-*` hosts through `kobe ssh-proxy` |
...
| `kobe config set <name> ...` | Create or replace a named target (`--default-pool` sets the pool `kobe ssh-proxy` falls back to) |
