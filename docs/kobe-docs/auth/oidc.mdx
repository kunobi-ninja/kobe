...
It requests the token and releases the lease when the job ends. The action
default audience is `kobe-system`; the policy above matches that. If you set
`audience` on the action, change the policy to the same value.
